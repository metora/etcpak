...
void Average( const uint8* data, v4i* a )
{
    ...
    uint16x8x2_t s0 = vzipq_u16(vaddq_s16(d0.val[0], d1.val[0]), uint16x8_t());
    uint16x8x2_t s1 = vzipq_u16(vaddq_s16(d0.val[1], d1.val[1]), uint16x8_t());
    uint16x8x2_t s2 = vzipq_u16(vaddq_s16(d2.val[0], d3.val[0]), uint16x8_t());
    uint16x8x2_t s3 = vzipq_u16(vaddq_s16(d2.val[1], d3.val[1]), uint16x8_t());
    ...
    uint16x8_t o0 = vcombine_u16(vqmovun_s32(a0), vqmovun_s32(a1));
    uint16x8_t o1 = vcombine_u16(vqmovun_s32(a2), vqmovun_s32(a3));
...
void CalcErrorBlock( const uint8* data, uint err[4][4] )
{
    ...
    uint32x4_t a0 = vandq_u8(vqaddq_u32(b2, b3), vdupq_n_u32(0x00FFFFFF));
    uint32x4_t a1 = vandq_u8(vqaddq_u32(b0, b1), vdupq_n_u32(0x00FFFFFF));
    uint32x4_t a2 = vandq_u8(vqaddq_u32(b1, b3), vdupq_n_u32(0x00FFFFFF));
    uint32x4_t a3 = vandq_u8(vqaddq_u32(b0, b2), vdupq_n_u32(0x00FFFFFF));
...
uint64 CheckSolid( const uint8* src )
{
    ...
    int32x4_t c0 = vceqq_s32(d0, c);
    int32x4_t c1 = vceqq_s32(d1, c);
    int32x4_t c2 = vceqq_s32(d2, c);
    int32x4_t c3 = vceqq_s32(d3, c);
...
void FindBestFit( uint64 terr[2][8], uint16 tsel[16][8], v4i a[8], const uint32* id, const uint8* data )
{
    for( size_t i=0; i<16; i++ )
    {
        ...
        // Taking the absolute value is way faster. The values are only used to sort, so the result will be the same.
        uint32x4_t error0 = vabsq_s32(vaddq_s32(pix, g_table256_SIMD[0]));
        uint32x4_t error1 = vabsq_s32(vaddq_s32(pix, g_table256_SIMD[1]));
        uint32x4_t error2 = vabsq_s32(vsubq_s32(pix, g_table256_SIMD[0]));
        uint32x4_t error3 = vabsq_s32(vsubq_s32(pix, g_table256_SIMD[1]));

        uint32x4_t index0 = vandq_u32(vcltq_u32(error1, error0), vdupq_n_u32(1));
        uint32x4_t minError0 = vminq_u32(error0, error1);

        uint32x4_t index1 = vsubq_s32(vdupq_n_u32(2), vcltq_u32(error3, error2));
        ...
        uint32x4_t squareErrorHigh = vshrq_n_u32(vqdmulhq_s32(minError, minError), 1);
        uint32x4x2_t squareErrorZip = vzipq_u32(squareErrorLow, squareErrorHigh);
        uint64x2x2_t squareError = { vreinterpretq_u32_u64(squareErrorZip.val[0]), vreinterpretq_u32_u64(squareErrorZip.val[1]) };
        ...
        // Taking the absolute value is way faster. The values are only used to sort, so the result will be the same.
        error0 = vabsq_s32(vaddq_s32(pix, g_table256_SIMD[2]));
        error1 = vabsq_s32(vaddq_s32(pix, g_table256_SIMD[3]));
        error2 = vabsq_s32(vsubq_s32(pix, g_table256_SIMD[2]));
        error3 = vabsq_s32(vsubq_s32(pix, g_table256_SIMD[3]));

        index0 = vandq_u32(vcltq_u32(error1, error0), vdupq_n_u32(1));
        minError0 = vminq_u32(error0, error1);

        index1 = vsubq_s32(vdupq_n_u32(2), vcltq_u32(error3, error2));
        ...
        squareErrorHigh = vshrq_n_u32(vqdmulhq_s32(minError, minError), 1);
        squareErrorZip = vzipq_u32(squareErrorLow, squareErrorHigh);
        squareError = { vreinterpretq_u32_u64(squareErrorZip.val[0]), vreinterpretq_u32_u64(squareErrorZip.val[1]) };
        squareError.val[0] = vaddq_u64(squareError.val[0], vld1q_u64(ter + 4));
        squareError.val[1] = vaddq_u64(squareError.val[1], vld1q_u64(ter + 6));
